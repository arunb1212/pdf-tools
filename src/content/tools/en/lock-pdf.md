---
tool: "lock-pdf"
locale: "en"
component: "LockPdf"
processing: "client"
slug: "lock-pdf"
title: "Lock PDF — add a password to a PDF, free and private"
shortTitle: "Lock PDF"
description: "Protect a PDF with a password. Encryption runs locally in your browser — your file is never uploaded, so it stays private."
faq:
  - q: "Is the password sent to a server?"
    a: "No. Both the PDF and the password are processed entirely in your browser with pdf-lib."
  - q: "What kind of encryption is used?"
    a: "The PDF standard encryption. The file can only be opened with the password you set."
  - q: "Can I lock a PDF I already locked before?"
    a: "Yes — just unlock it first by entering your current password, then set a new one."
---

# Lock PDF

Add password protection to a PDF — with no upload and no watermarks.
